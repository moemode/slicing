...
function pruneProgram(prog: string, relevantLocs: any[], relevant_vars: string | unknown[]) {
    let ast = parse(prog);
    visit(ast,  {
...
function prune(progInPath: PathOrFileDescriptor, progOutPath: PathOrFileDescriptor, 
    ...
    const nodesAtCriterion =  graph.nodes().filter((e, i) => SourceLocation.in_between_inclusive(slicingCriterion, e.data("loc")));
    ...
    const relevantVars: string[] = Array.from(new Set(nodes.map(node => node.data("varname")).filter((x: any) => x)));
